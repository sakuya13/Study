...
processSymbols pairs = execState (insertSymbols pairs) empty
...
processInput symbs table = evalState (lookupSymbols symbs) table
...
process pairs symbs = 
    let table = processSymbols pairs
        mInfo = processInput symbs table
    in  mInfo
...
insertSymbols ((symb, info):pairs) = do
    insertSymbol symb info
    insertSymbols pairs
...
lookupSymbol symb = do
    table <- get
    return $ Data.Map.lookup symb table
...
lookupSymbols (symb:symbs) = do
    mi <- lookupSymbol symb
    mis <- lookupSymbols symbs
    return (mi:mis)
